<template>
  <el-dialog v-model="drawerVisible" :destroy-on-close="true" :width="1100" :title="`${drawerProps.title}`">
    ...
    >
      <el-form-item label="图片名称" prop="banner_title">
        <el-input v-model="drawerProps.rowData!.name" placeholder="请输入图片名称" clearable></el-input>
      </el-form-item>
      <el-form-item label="图片简介" prop="intro">
        <el-input type="textarea" rows="2" v-model="drawerProps.rowData!.intro" placeholder="请输入图片简介" clearable></el-input>
      </el-form-item>
      <el-form-item label="图片" prop="imgs">
        <UploadImg v-model:image-url="drawerProps.rowData!.imgs" width="250px">
          <template #empty>
...
const rules = reactive({
  ...
  imgs: [{ required: true, message: "请上传图片名称", trigger: "blur" }]
});
